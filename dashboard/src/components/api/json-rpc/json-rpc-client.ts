...
export class JsonRpcClient {
   ...
   */
  constructor ($websocket: ng.websocket.IWebSocketProvider, $location: ng.ILocationService, $interval : ng.IIntervalService,
               proxySettings : string, userDashboardConfig: any) {
    ...
    if (inDevMode) {
      ...
      wsUrl = proxySettings.replace('http', 'ws') + '/wsmaster/websocket';
    } else {
      ...
      wsUrl = wsProtocol + '://' + $location.host() + ':' + $location.port() + '/wsmaster/websocket';
    }
    this.entrypoint = wsUrl;
    this.bus = null;
 /*   this.remoteBus = null;*/
  }
